...
describe('Toast', () => {
    describe('MM-T1784 View new messages toast', () => {
        let otherUser;
        let testTeam;
        ...
        it('should see a toast with Jump to recent messages button', () => {
        ...
        it('should see a toast with number of unread messages in the toast if the bottom is not in view', () => {
        ...
        it('should show the mobile view version of the toast', () => {
            visitTownSquareAndWaitForPageToLoad();
            const numberOfPost = 2;
            scrollUpAndPostAMessage(otherUser, townsquareChannelId, numberOfPost);
            ...
            cy.get('.toast__visible').should('be.visible').within(() => {
                ...
                cy.get('.toast__message>span').should('be.visible').first().contains(`${numberOfPost} new messages since `).find('time').should('be.visible');
        ...
        it('marking channel as unread should reappear new message toast', () => {
            visitTownSquareAndWaitForPageToLoad();
            ...
            cy.getNthPostId(-oldPostNumber).then((postId) => {
                ...
                cy.get('div.toast').should('be.visible');
                cy.get('div.toast__message>span').should('be.visible').first().contains(`${oldPostNumber} new messages since`);
                ...
                Cypress._.times(2, (num) => {
                    ...
                    cy.get('div.toast__message>span').should('be.visible').first().contains(`${oldPostNumber + num + 1} new messages since`);
                });
            });
        });
    });
...
function scrollUpAndPostAMessage(sender, channelId, nTimes = 1) {
    scrollUp();

    // # Post message for nth times
    const randomId = getRandomId();
