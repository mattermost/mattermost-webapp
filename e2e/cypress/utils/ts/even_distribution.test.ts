...
describe('getTestFilesIdentifier', () => {
    it('should return expected output', () => {
        ...
        testCases.forEach((testCase) => {
            ...
            (expect(testCase.outStart) as any).toEqual(actual.start);
            (expect(testCase.outEnd) as any).toEqual(actual.end);
            (expect(testCase.outCount) as any).toEqual(actual.count);
        });
    });
});
