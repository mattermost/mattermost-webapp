...
function verifyCreatePublicChannel(teamName, testUsers) {
    for (const testUser of testUsers) {
        ...
        cy.findByRole('dialog', {name: 'Create a new channel'}).within(() => {
            ...
            cy.get('#O.public-private-selector-button').should(canCreate ? 'not.have.class' : 'have.class', 'disabled');

            // * Verify if creating a private channel is disabled or not
            cy.get('#P.public-private-selector-button').should(isSysadmin ? 'not.have.class' : 'have.class', 'disabled');
        });
    }
}
