...
type ChainableChannel = Cypress.Chainable<Cypress.Channel>
...
function createChannel(channelType, teamId, userToAdd = null): ChainableChannel {
    ...
    return cy.externalRequest({
        ...
        if (userToAdd) {
            ...
            }) as unknown as ChainableChannel;
        }

        // # Explicitly wait to give some time to index before searching
        cy.wait(TIMEOUTS.TWO_SEC);
        return cy.wrap(channel) as unknown as ChainableChannel;
    });
}
...
function getTestUsers(): Record<string, any> {
...
function createPrivateChannel(teamId, userToAdd = null): ChainableChannel {
    // # Create a private channel as sysadmin
    return createChannel('P', teamId, userToAdd);
}

function createPublicChannel(teamId, userToAdd = null): ChainableChannel {
