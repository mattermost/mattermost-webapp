...
describe('System Console > Team Statistics', () => {
    ...
    it('MM-T906 Team Statistics displays expected content categories', () => {
        ...
        cy.get('.admin-console__content .row').should('have.length', 3);
        ...
        cy.get('.admin-console__content .row').eq(0).find('.title').eq(0).should('contain', 'Total Posts');
        cy.get('.admin-console__content .row').eq(1).find('.title').eq(0).should('contain', 'Active Users With Posts');
        cy.get('.admin-console__content .row').eq(2).find('.title').eq(0).should('contain', 'Recent Active Users');
        cy.get('.admin-console__content .row').eq(2).find('.title').eq(1).should('contain', 'Newly Created Users');
