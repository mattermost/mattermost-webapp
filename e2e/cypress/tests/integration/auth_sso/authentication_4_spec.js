...
describe('Authentication', () => {
    ...
    it('MM-T1770 - Default password settings', () => {
        cy.apiGetClientLicense().then(({isCloudLicensed}) => {
            ...
            cy.findByRole('checkbox', {name: 'At least one lowercase letter'}).click().should('be.checked');
            cy.findByRole('checkbox', {name: 'At least one uppercase letter'}).click().should('be.checked');
            cy.findByRole('checkbox', {name: 'At least one number'}).click().should('be.checked');
            cy.findByRole('checkbox', {name: 'At least one symbol (e.g. "~!@#$%^&*()")'}).click().should('be.checked');
