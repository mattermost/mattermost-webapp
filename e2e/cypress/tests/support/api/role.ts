...
function getRoleByName(name: string): ChainableT<{name: Role}> {
    return cy.request({
...
function apiGetRolesByNames(names?: string[]): ChainableT<{roles: Role[]}> {
    return cy.request({
        headers: {'X-Requested-With': 'XMLHttpRequest'},
...
function apiPatchRole(roleID: string, patch: Partial<Role>): ChainableT<{role: Role}> {
    return cy.request({
        headers: {'X-Requested-With': 'XMLHttpRequest'},
