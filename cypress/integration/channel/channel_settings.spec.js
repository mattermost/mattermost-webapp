...
describe('Channel Settings', () => {
    ...
    it('C15052 All channel types have appropriate close button', () => {
        cy.get('#publicChannel').parent().parent().children('li:not(:first-child)').not('#moreButton').each(($el) => {
            cy.wrap($el).find('a').find('span.btn-close').should('have.length', 0);
        });

        cy.get('#privateChannel').parent().parent().children('li:not(:first-child)').not('#moreButton').each(($el) => {
            cy.wrap($el).find('a').find('span.btn-close').should('have.length', 0);
        });
        ...
        cy.get('#directChannel').parent().parent().children('li:not(:first-child)').not('#moreButton').each(($el) => {
    ...
    after(() => {
        ...
        cy.get('#directChannel').parent().parent().children('li:not(:first-child)').not('#moreButton').
            first().get('span.btn-close').click({force: true});
    });
});