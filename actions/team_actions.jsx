...
export function checkIfTeamExists(teamName, onSuccess, onError) {
    TeamActions.checkIfTeamExists(teamName)(dispatch, getState).then(exists => {
...
export function createTeam(team, onSuccess, onError) {
    TeamActions.createTeam(team)(dispatch, getState).then(rteam => {
        if (rteam && onSuccess) {
...
export function updateTeam(team, onSuccess, onError) {
    TeamActions.updateTeam(team)(dispatch, getState).then(rteam => {
        if (rteam && onSuccess) {
            browserHistory.push('/' + rteam.name + '/channels/town-square');
            onSuccess(rteam);
        } else if (rteam == null && onError) {
...
export function removeUserFromTeam(teamId, userId, success, error) {
    TeamActions.removeUserFromTeam(teamId, userId)(dispatch, getState).then(data => {
...
export function updateTeamMemberRoles(teamId, userId, newRoles, success, error) {
    TeamActions.updateTeamMemberRoles(teamId, userId, newRoles)(dispatch, getState).then(data => {
        if (data && success) {
            success();
        } else if (data == null && error) {
...
export function addUserToTeamFromInvite(data, hash, inviteId, success, error) {
    Client4.addToTeamFromInvite(hash, data, inviteId)
        .then(member => {
            TeamActions.getTeam(member.team_id)(dispatch, getState).then(team => {
                dispatch({
...
export function addUsersToTeam(teamId, userIds, success, error) {
    TeamActions.addUsersToTeam(teamId, userIds)(dispatch, getState).then(teamMembers => {
...
export function getInviteInfo(inviteId, success, error) {
    Client4.getTeamInviteInfo(inviteId)
        .then(inviteData => {
            if (success) {
                success(inviteData);
            }
...
export function inviteMembers(data, success, error) {
    ...
    TeamActions.sendEmailInvitesToTeam(TeamStore.getCurrentId(), emails)(dispatch, getState).then(result => {
        if (result && success) {
            success();
...
export function getTeamsForUser(userId, success, error) {
    TeamActions.getTeamsForUser(userId)(dispatch, getState).then(result => {
        if (result && success) {
            success(result);
        } else if (result == null && error) {
...
export function getTeamMembersForUser(userId, success, error) {
    TeamActions.getTeamMembersForUser(userId)(dispatch, getState).then(result => {
        if (result && success) {
            success(result);
        } else if (result == null && error) {
