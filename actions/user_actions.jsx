...
export async function createUserWithInvite(user, data, token, inviteId, success, error) {
    const {data: resp, error: err} = await UserActions.createUser(user, data, token, inviteId)(dispatch, getState);
    if (resp && success) {
        success(resp);
    } else if (err && error) {
        error({id: err.server_error_id, ...err});
