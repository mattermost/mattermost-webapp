...
export function loadMeAndConfig(callback) {
    getClientConfig()(store.dispatch, store.getState).then(config => {
        ...
        if (global.window && global.window.analytics) {
            global.window.analytics.identify(
                global.window.mm_config.DiagnosticId,
                {},
                {
                    context: {
                        ip: '0.0.0.0'
        ...
        Promise.all([
            loadMe(),
            getLicenseConfig()(store.dispatch, store.getState).then(license => {
...
export function switchFromLdapToEmail(email, password, token, ldapPassword, success, error) {
    UserActions.switchLdapToEmail(ldapPassword, email, password, token)(dispatch, getState).then(data => {
        if (data) {
            if (data.follow_link) {
...
export function loadProfilesAndTeamMembers(page, perPage, teamId = TeamStore.getCurrentId(), success) {
    UserActions.getProfilesInTeam(teamId, page, perPage)(dispatch, getState).then(data => {
...
export function loadProfilesAndTeamMembersAndChannelMembers(
    ...
    UserActions.getProfilesInChannel(channelId, page, perPage)(dispatch, getState).then(data => {
        loadTeamMembersForProfilesList(data, teamId, () => {
...
export function loadProfilesWithoutTeam(page, perPage, success) {
    UserActions.getProfilesWithoutTeam(page, perPage)(dispatch, getState).then(data => {
...
function loadTeamMembersForProfiles(userIds, teamId, success, error) {
    getTeamMembersByIds(teamId, userIds)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
...
export function searchUsers(term, teamId = TeamStore.getCurrentId(), options = {}, success) {
    UserActions.searchProfiles(term, {team_id: teamId, ...options})(dispatch, getState).then(data => {
        loadStatusesForProfilesList(data);
...
export function searchUsersNotInTeam(term, teamId = TeamStore.getCurrentId(), options = {}, success) {
    UserActions.searchProfiles(term, {not_in_team_id: teamId, ...options})(dispatch, getState).then(data => {
        loadStatusesForProfilesList(data);
...
export function autocompleteUsersInChannel(username, channelId, success) {
    ...
    UserActions.autocompleteUsers(username, teamId, channelId)(dispatch, getState).then(data => {
        if (success) {
            success(data);
        }
    });
...
export function autocompleteUsersInTeam(username, success) {
    UserActions.autocompleteUsers(username, TeamStore.getCurrentId())(dispatch, getState).then(data => {
        if (success) {
            success(data);
        }
    });
...
export function autocompleteUsers(username, success) {
    UserActions.autocompleteUsers(username)(dispatch, getState).then(data => {
        if (success) {
            success(data);
        }
    });
...
export function updateUser(user, type, success, error) {
    UserActions.updateMe(user)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
...
export function generateMfaSecret(success, error) {
    UserActions.generateMfaSecret(UserStore.getCurrentId())(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
...
export function updateUserNotifyProps(props, success, error) {
    UserActions.updateMe({notify_props: props})(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
            const serverError = getState().requests.users.updateMe.error;
...
export function updateUserRoles(userId, newRoles, success, error) {
    UserActions.updateUserRoles(userId, newRoles)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
...
export function activateMfa(code, success, error) {
    UserActions.updateUserMfa(UserStore.getCurrentId(), true, code)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
            const serverError = getState().requests.users.updateUser.error;
...
export function deactivateMfa(success, error) {
    UserActions.updateUserMfa(UserStore.getCurrentId(), false)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
            const serverError = getState().requests.users.updateUser.error;
...
export function checkMfa(loginId, success, error) {
    ...
    UserActions.checkMfa(loginId)(dispatch, getState).then(data => {
...
export function updateActive(userId, active, success, error) {
    UserActions.updateUserActive(userId, active)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
            const serverError = getState().requests.users.updateUser.error;
...
export function updatePassword(userId, currentPassword, newPassword, success, error) {
    UserActions.updateUserPassword(userId, currentPassword, newPassword)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
            const serverError = getState().requests.users.updateUser.error;
...
export function verifyEmail(token, success, error) {
    UserActions.verifyUserEmail(token)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
...
export function resetPassword(token, password, success, error) {
    UserActions.resetUserPassword(token, password)(dispatch, getState).then(data => {
        if (data) {
...
export function resendVerification(email, success, error) {
    UserActions.sendVerificationEmail(email)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
            const serverError = getState().requests.users.verifyEmail.error;
...
export function loginById(userId, password, mfaToken, success, error) {
    UserActions.loginById(userId, password, mfaToken)(dispatch, getState).then(ok => {
        if (ok && success) {
            success();
        } else if (!ok && error) {
...
export function createUserWithInvite(user, data, emailHash, inviteId, success, error) {
    UserActions.createUser(user, data, emailHash, inviteId)(dispatch, getState).then(resp => {
        if (resp && success) {
            success(resp);
...
export function webLogin(loginId, password, token, success, error) {
    UserActions.login(loginId, password, token)(dispatch, getState).then(ok => {
        if (ok && success) {
            success();
        } else if (!ok && error) {
            const serverError = getState().requests.users.login.error;
...
export function webLoginByLdap(loginId, password, token, success, error) {
    UserActions.login(loginId, password, token, true)(dispatch, getState).then(ok => {
        if (ok && success) {
            success();
        } else if (!ok && error) {
            const serverError = getState().requests.users.login.error;
...
export function getAuthorizedApps(success, error) {
    Client4.getAuthorizedOAuthApps(getState().entities.users.currentUserId)
        .then(authorizedApps => {
            if (success) {
...
export function deauthorizeOAuthApp(appId, success, error) {
    Client4.deauthorizeOAuthApp(appId)
        .then(() => {
            if (success) {
                success();
            }
...
export function uploadProfileImage(userPicture, success, error) {
    UserActions.uploadProfileImage(Selectors.getCurrentUserId(getState()), userPicture)(
        dispatch,
        getState
    ).then(data => {
        if (data && success) {
...
export function loadProfiles(page, perPage, success) {
    UserActions.getProfiles(page, perPage)(dispatch, getState).then(data => {
        if (success) {
            success(data);
        }
    });
...
export function savePreferences(prefs, callback) {
    const currentUserId = UserStore.getCurrentId();
    savePreferencesRedux(currentUserId, prefs)(dispatch, getState).then(() => callback());
}
...
export function sendPasswordResetEmail(email, success, error) {
    UserActions.sendPasswordResetEmail(email)(dispatch, getState).then(data => {
        if (data && success) {
            success(data);
        } else if (data == null && error) {
            const serverError = getState().requests.users.passwordReset.error;
