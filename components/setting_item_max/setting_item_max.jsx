...
export default class SettingItemMax extends React.PureComponent {
    ...
    componentDidMount() {
        if (this.settingList.current) {
            const focusableElements = this.settingList.current.querySelectorAll('.btn:not(.save-button):not(.btn-cancel), input.form-control, input[type=radio]:checked, input[type=checkbox], select, textarea, [tabindex]:not([tabindex="-1"])');
