...
export default class WebrtcController extends React.Component {
    ...
    handleMakeOffer() {
        if (UserStore.getStatus(this.props.userId) === UserStatuses.OFFLINE) {
            ...
            const connectingMsg = <FormattedMessage id="calling_screen" defaultMessage="Calling" />;
    ...
    onFailed() {
        ...
        this.setState({
            ...
            isRemoteMuted: false,
            error: (
                <FormattedMessage id="webrtc.failed" defaultMessage="There was a problem connecting the video call" />
            )
        });
    ...
    onConnectCall() {
        WebrtcActions.webrtcToken(
            info => {
                const connectingMsg = <FormattedMessage id="connecting_screen" defaultMessage="Connecting" />;
        ...
        if (this.state.isCalling) {
            buttons = (
                ...
                >
                    <circle cx="24" cy="24" r="34">
                        <title>
                            <FormattedMessage id="webrtc.cancel" defaultMessage="Cancel Call" />
                        </title>
                    </circle>
                    <path
            ...
            buttons = (
                ...
                >
                    <circle cx="24" cy="24" r="34">
                        <title>
                            <FormattedMessage id="webrtc.call" defaultMessage="Call" />
                        </title>
                    </circle>
                    <path
            ...
            let audioTitle = <FormattedMessage id="webrtc.mute_audio" defaultMessage="Mute" />;

            let videoTitle = <FormattedMessage id="webrtc.pause_video" defaultMessage="Turn off Video" />;

            if (this.state.isMuted) {
                audioOnClass = onClass;
                audioOffClass = offClass;
                audioTitle = <FormattedMessage id="webrtc.unmute_audio" defaultMessage="Unmute" />;
            }

            if (this.state.isPaused) {
                videoOnClass = onClass;
                videoOffClass = offClass;
                videoTitle = <FormattedMessage id="webrtc.unpause_video" defaultMessage="Turn on Video" />;
            }

            buttons = (
                <div ref="icons" className="webrtc-icons hidden">
                    <svg
                        id="mute-audio"
                        className="webrtc-icons__call"
                        xmlns="http://www.w3.org/2000/svg"
                    ...
                    >
                        <circle cx="24" cy="24" r="34">
                            <title>
                                <FormattedMessage id="webrtc.hangup" defaultMessage="Hang up" />
                            </title>
                        </circle>
                        <path
    ...
    render() {
        ...
        if (this.state.isRemoteMuted) {
            remoteMute = (
                <div className="webrtc__remote-mute">
                    <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="-10 -10 68 68">
                        <path
                            className="off"
                            transform="scale(0.6), translate(17,18)"
                            d="M38 22h-3.4c0 1.49-.31 2.87-.87 4.1l2.46 2.46C37.33 26.61 38 24.38 38 22zm-8.03.33c0-.11.03-.22.03-.33V10c0-3.32-2.69-6-6-6s-6 2.68-6 6v.37l11.97 11.96zM8.55 6L6 8.55l12.02 12.02v1.44c0 3.31 2.67 6 5.98 6 .45 0 .88-.06 1.3-.15l3.32 3.32c-1.43.66-3 1.03-4.62 1.03-5.52 0-10.6-4.2-10.6-10.2H10c0 6.83 5.44 12.47 12 13.44V42h4v-6.56c1.81-.27 3.53-.9 5.08-1.81L39.45 42 42 39.46 8.55 6z"
        ...
        if (calling || this.state.isAnswering) {
            if (calling) {
                audio = <audio ref="ring" src={ring} autoPlay={true} />;
            }

            connecting = (
                <div className="connecting">
                    <ConnectingScreen position="absolute" message={this.state.connectingMsg} />
                    {audio}
                </div>
            );
        }
            ...
            <div className="post-right__container">
                ...
                <div className="sidebar-right__body">
                    ...
                    <div className="post-right__scroll">
                        <div id="videos">
                            {remoteMute}
                            <div id="main-video" className={remoteVideoHidden} autoPlay={true}>
                                <video ref="main-video" autoPlay={true} />
                            </div>
                            <div id="local-video" className={localVideoHidden}>
                                <video ref="local-video" autoPlay={true} muted={true} />
                            </div>
                            <div className={remoteImageHidden}>{remoteImage}</div>
                            <div className={localImageHidden}>{localImage}</div>
                        </div>
                        {error}
                        {connecting}
                        <div className="webrtc-buttons">{buttons}</div>
                    </div>
                </div>
            </div>
        );
