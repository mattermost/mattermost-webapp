...
type Props = {
    ...
    users: Object[];
    serverError: string;
    actions: { getComplianceReports: () => any, createComplianceReport: (item: any) => any};
}
...
export default class ComplianceReports extends React.PureComponent<Props, State> {
    ...
    runReport = (e: { preventDefault: () => void; }) => {
        e.preventDefault();
    ...
    getDateTime(millis: string | number | Date) {
    ...
    render() {
        ...
        if (this.state.loadingReports) {
            ...
            for (var i = 0; i < this.props.reports.length; i++) {
                const report: any = this.props.reports[i];
                ...
                const profile: any = this.props.users[report.user_id];
                if (profile) {
                    user = profile.email;
                }
