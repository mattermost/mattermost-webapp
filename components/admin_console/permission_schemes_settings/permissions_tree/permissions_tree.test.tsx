...
describe('components/admin_console/permission_schemes_settings/permission_tree', () => {
    ...
    test('should map groups in the correct order', () => {
        ...
        expect((groups[0] as Group | Permission).id).toStrictEqual('teams');
        expect((groups[6] as Group | Permission).id).toStrictEqual('posts');
        expect((groups[7] as Group | Permission).id).toStrictEqual('integrations');
        expect((groups[8] as Group | Permission).id).toStrictEqual('manage_shared_channels');
        expect((groups[9] as Group | Permission).id).toStrictEqual('custom_groups');
    });
    ...
    describe('should show playbook permissions', () => {
        describe('for non-enterprise license', () => {
            ['', LicenseSkus.E10, LicenseSkus.Starter, LicenseSkus.Professional].forEach((licenseSku) => test(licenseSku, () => {
                ...
                const groups: Omit<Permissions, string> = wrapper.find(PermissionGroup).first().prop('permissions');
                expect((groups[3] as Group | Permission).id).toStrictEqual('playbook_public');
                expect((groups[4] as Group | Permission).id).toStrictEqual('runs');
            }));
        });
        ...
        describe('for enterprise license', () => {
            [LicenseSkus.E20, LicenseSkus.Enterprise].forEach((licenseSku) => test(licenseSku, () => {
                ...
                const groups = wrapper.find(PermissionGroup).first().prop('permissions');
                expect((groups[3] as Group | Permission).id).toStrictEqual('playbook_public');
                expect((groups[4] as Group | Permission).id).toStrictEqual('playbook_private');
                expect((groups[5] as Group | Permission).id).toStrictEqual('runs');
            }));
        });
    });
});
