...
export function getChannelURL(channel, teamId) {
    ...
    if (channel && (channel.type === Constants.DM_CHANNEL || channel.type === Constants.GM_CHANNEL)) {
        ...
        const team = getTeam(state, teamId);
        notificationURL = getTeamRelativeUrl(team) + `/channels/${Constants.DEFAULT_CHANNEL}`;
    } else {
        notificationURL = getCurrentRelativeTeamUrl(state) + `/channels/${Constants.DEFAULT_CHANNEL}`;
    }
