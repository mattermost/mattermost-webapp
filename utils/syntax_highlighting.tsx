...
export async function highlight(lang: string, code: string, addLineNumbers = false) {
    const language = getLanguageFromNameOrAlias(lang);

    if (language) {
        try {
            const languageHighlighting = await import(`highlight.js/lib/languages/${language}.js`);
            hlJS.registerLanguage(language, languageHighlighting.default);
